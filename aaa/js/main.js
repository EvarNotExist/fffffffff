 function showNotes(typenote) {
    BX24.init(function(){
        var html = '';
        BX24.callBatch({
            get_section: ['entity.section.get', {ENTITY: 'librarynote', FILTER: {NAME: typenote}}],
            get_note: ['entity.item.get', {ENTITY: 'librarynote', SORT: {DATE_ACTIVE_FROM: 'DESC', ID: 'ASC'}}],
            get_user: ['user.get'],
    ...
    BX24.init(function(){
        ...
        BX24.callMethod('entity.item.get', {
            ...
            for (var i = 0; i < Math.round(result.answer.total); i++) {
                ...
                htmlsec += '<td>' + item[i].PROPERTY_VALUES.description + '</td>';
                htmlsec += '<td>' + item[i].PROPERTY_VALUES.isbn + '</td>';
                htmlsec += '<td>' + item[i].PROPERTY_VALUES.category + '</td>';
                htmlsec += '<td><img src="' + item[i].DETAIL_PICTURE + '" ></td>';
...
function showAcBooks() {
    BX24.init(function(){
        var select = '';
        BX24.callBatch({
            get_booktwo: ['entity.item.get', {ENTITY: 'librarybook', SORT: {ID: 'ASC'}}],
            get_notetwo: ['entity.item.get', {ENTITY: 'librarynote'}]
        }, function(result){
...
function addBook() {
    ...
    var toaddbookdescription = $("#toaddbookdescription").val();
    var toaddbookISBN = $("#toaddbookISBN").val();
    var toaddbookcategory = $("#toaddbookcategory").val();

    BX24.init(function(){
        BX24.callMethod('entity.item.add', {
            ENTITY: 'librarybook',
            DATE_ACTIVE_FROM: new Date(),
            DETAIL_PICTURE: document.getElementById('toaddbookpicture'),
            NAME: 'OneOfThis',
            PROPERTY_VALUES: {
                name: toaddbookname,
                ...
                description: toaddbookdescription,
                isbn: toaddbookISBN,
                category: toaddbookcategory
            }
        }, function(){
            showBooks();
        });
